...
* `gasket-dkms.deb`: It's a package containing the device driver for Edge TPUs, required for PCIe devices only. It enables the Linux kernel to recognize and interact with the Edge TPU hardware.
* `libedgeTPU.deb` (or `edgedpu_runtime.zip`): That package contains the core Edge TPU library `libedgetpu.so.1` that provides low-level access to the Edge TPU hardware. It enables communication between software and Edge TPU.
